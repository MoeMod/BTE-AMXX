...
new const WEAPON_LIMIT_RARE[][] = {"m1887","m134","m95","xm2010","stg44","m14ebr"}
new const KILL_AIM[] = {"","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4"}
new g_msgScenario
...
public update_player_scenario(id)
{
	...
	new sprname[32]
	format(sprname,31,"gd_remain%d",/*KILL_AIM[g_level[1][id]]*/4-g_level[0][id])
...
public logevent_round_start()
{
	...
	for(new i=1;i<33;i++)
	{
		if(is_user_alive(i)) server_cmd("sv_noroundend 1")
...
public set_end_round(killer)
{
	...
	if (g_map_continue)
		set_task(8.0, "change_map", TASK_CHANGE_MAP)
	...
	client_print(0,print_center,"%L",LANG_PLAYER,"BTE_GUNDEATH_HEKILL",name)
	bte_fun_process_data(killer,1)
}
...
public message_DeathMsg()
{
	...
	if (contain(weapon, "grenade")>-1 && killer!=victim)
	{
	...
	if(0<killer<33 && (cs_get_user_team(killer)!=cs_get_user_team(victim)))
	{
		...
		{
			g_level[0][killer] += 1
			if (g_level[0][killer] == /*KILL_AIM[g_level[1][killer]]*/4)
			{
				g_level[1][killer] += 1
...
{
	if(!is_user_bot(id)) bte_wpn_give_named_wpn(id,"knife")
}
public give_he(id)
{
	new name[32]
...
public give_weapons(id)
{
	...
	if (g_level[1][id] == MAX_LEVEL)
	{
		bte_wpn_give_named_wpn(id,"glock18")
		if (g_he[id])
			bte_wpn_give_named_wpn(id,"hegrenade")
	}
	else
	{
		bte_wpn_give_named_wpn(id,g_weapon[id])
