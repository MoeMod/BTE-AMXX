...
public Task_RespawnSupplyBox(taskid)
{
	...
	if (g_supplybox_count>=SUPPLYBOX_MAX || !g_newround || g_endround)
	{
		return
	}
...
public Task_CountDown()
{
	...
	else
	{
		if (g_count_down<=10)
		{
			PlaySound(0, SND_COUNT[g_count_down - 1])
		}

		new sec[3];
		format(sec, charsmax(sec), "%d", g_count_down);
		ClientPrint(0, HUD_PRINTCENTER, "#CSBTE_ZombiSelectCount", sec);
	}
...
public Task_ZombieRespawnEffect(taskid)
{
	...
	if (task_exists(id+TASK_ZOMBIE_RESPAWN_EF))
...
public Task_RoundEngBgm(taskid)
{
	PlaySound(0, SND_ROUND_END);
}